...
磁碟分割區，可以分四個區：
- ESP(0)分割區 ：
	檔案系統類型為FAT32，分配1.0GB。用於Linux系統的 /boot開機分割區，後續啟動 Ubuntu 系統的開機檔(.efi)將會放在這個分割區下的EFI目錄
	想知道此分區有多重要[點選跳轉 OS 概念](#OS%20概念)
- 分割區(1)：
	檔案系統類型為Linux swap partition，分配16GB。此分割區用於Linux系統的swap交換空間。
- 分割區(2)：
	檔案系統類型為EXT4，分配64GB。用於Linux系統的 “/” 目錄。
- 分割區(3) ：
	檔案系統類型為EXT4，分配 100GB。用於Linux系統的 “/home” 目錄。
